          ...
          ,'<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">'
            ...
            ,'" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
              ...
              ,'{{# } else { }}'
                ...
                ,'{{# if(isSort){ }}'
                  ,'<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc" title="昇順"></i><i class="layui-edge layui-table-sort-desc" title="降順"></i></span>'
                ,'{{# } }}'
              ,'{{# } }}'
            ,'</div>'
          ,'</th>'
        ...
        ,success: function(res){
          ...
          if(res[response.statusName] != response.statusCode){
            ...
            that.errorView(
              ...
              ('データフォーマット不正、成功の場合コードは"'+ response.statusName +'": '+ response.statusCode)
            );
          } else {
    ...
    that.layTool.on('click', '*[lay-event]', function(e){
      ...
      switch(events){
        ...
        case 'LAYTABLE_EXPORT': //导出
          if(device.ie){
            layer.tips('出力機能はＩＥで保障されない、Ｃｈｒｏｍｅ等で出力してください', this, {
              tips: 3
            })
          } else {
            openPanel({
              list: function(){
                return [
                  '<li data-type="csv">・ Csv へ出力</li>'
                  ,'<li data-type="xls">・ Excel へ出力</li>'
                ].join('')
              }()
        ...
        case 'LAYTABLE_PRINT': //打印
          var printWin = window.open('印刷ウィンドウ', '_blank')
          ,style = ['<style>'
