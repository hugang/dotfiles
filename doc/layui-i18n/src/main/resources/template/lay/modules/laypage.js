...
layui.define(function(exports){
  ...
  Class.prototype.view = function(){
    ...
    config.prev = 'prev' in config ? config.prev : '${laypage_prev_page}'; //上一页文本
    config.next = 'next' in config ? config.next : '${laypage_next_page}'; //下一页文本
    ...
    ,views = {
      ...
      ,page: function(){
        ...
        if(index > 1 && config.first !== false && groups !== 0){
          pager.push('<a href="javascript:;" class="layui-laypage-first" data-page="1"  title="${laypage_first_page}">'+ (config.first || 1) +'</a>');
        }
        ...
        if(config.pages > groups && config.pages > end && config.last !== false){
          ...
          if(groups !== 0){
            pager.push('<a href="javascript:;" class="layui-laypage-last" title="${laypage_last_page}"  data-page="'+ config.pages +'">'+ (config.last || config.pages) +'</a>');
          }
        }
      ...
      //数据总数
      ,count: '<span class="layui-laypage-count">${laypage_all} '+ config.count +' ${laypage_unit}</span>'
      ...
      //跳页区域
      ,skip: function(){
        return ['<span class="layui-laypage-skip">${laypage_to_page}'
          ,'<input type="text" min="1" value="'+ config.curr +'" class="layui-input">'
          ,'${laypage_page}<button type="button" class="layui-laypage-btn">${confirm}</button>'
        ,'</span>'].join('');
      }()
    };
