          ...
          ,'<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">'
            ...
            ,'" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
              ...
              ,'{{# } else { }}'
                ...
                ,'{{# if(isSort){ }}'
                  ,'<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc" title="${table_sort_asc}"></i><i class="layui-edge layui-table-sort-desc" title="${table_sort_desc}"></i></span>'
                ,'{{# } }}'
              ,'{{# } }}'
            ,'</div>'
          ,'</th>'
  ...
  Class.prototype.pullData = function(curr){
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,success: function(res){
          ...
          if(res[response.statusName] != response.statusCode){
            ...
            that.errorView(
              ...
              ('${table_data_format_error}"'+ response.statusName +'": '+ response.statusCode)
            );
          } else {
        ...
        ,error: function(e, m){
          that.errorView('${table_api_error}：'+ m);
    ...
    that.layTool.on('click', '*[lay-event]', function(e){
      ...
      switch(events){
        ...
        case 'LAYTABLE_EXPORT': //导出
          if(device.ie){
            layer.tips('${table_export_ie_support}', this, {
              tips: 3
            })
          } else {
            openPanel({
              list: function(){
                return [
                  '<li data-type="csv">${table_export_prefix} Csv ${table_export_file}</li>'
                  ,'<li data-type="xls">${table_export_prefix} Excel ${table_export_file}</li>'
                ].join('')
              }()
        ...
        case 'LAYTABLE_PRINT': //打印
          var printWin = window.open('${table_print_window}', '_blank')
          ,style = ['<style>'
