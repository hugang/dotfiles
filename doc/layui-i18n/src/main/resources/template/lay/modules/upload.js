  ...
  Class.prototype.upload = function(files, type){
    ...
    ,iframeSend = function(){
      ...
      Class.timer = setInterval(function() {
        ...
        try {
          ...
          that.msg('${upload_api_response_error}');
          clearInterval(Class.timer);
          error();
        }
        if(res){
    ...
    ,done = function(index, res){
      ...
      if(typeof res !== 'object'){
        try {
          ...
          res = {};
          return that.msg('${upload_api_response_msg}');
        }
      }
    ...
    switch(options.accept){
      ...
      default: //图片文件
        ...
        if(check){
          that.msg('${upload_unsupported_image}');
          return elemFile.value = '';
        }
      break;
    }
    ...
    if(options.number && that.fileLength > options.number){
      return that.msg('${upload_maxi_file_count}'+ options.number);
    }
    ...
    if(options.size > 0 && !(device.ie && device.ie < 10)){
      ...
      if(limitSize) return that.msg('${upload_maxi_file_size}'+ limitSize);
    }
    send();
  };
  ...
  Class.prototype.events = function(){
    ...
    ,setChooseText = function(files, filename){
      ...
      ,value = files.length > 1 
        ? files.length + '${upload_selected_file_count}'
